...
let
  nodejsPath = "${nixpkgsPath}/pkgs/development/web/nodejs";
  buildNodejs = callPackage "${nodejsPath}/nodejs.nix" {
    inherit openssl;
    python = python3;
  };
in
  ...
  patches =
    (
      if (stdenv.hostPlatform.emulatorAvailable buildPackages) then
        [
          "${nodejsPath}/configure-emulator.patch"
        ]
      else
        [
          (fetchpatch2 {
    ...
    ++ [
      "${nodejsPath}/configure-armv6-vfpv2.patch"
      "${nodejsPath}/disable-darwin-v8-system-instrumentation-node19.patch"
      "${nodejsPath}/bypass-darwin-xcrun-node16.patch"
      "${nodejsPath}/node-npm-build-npm-package-logic.patch"
      "${nodejsPath}/use-correct-env-in-tests.patch"
      "${nodejsPath}/bin-sh-node-run-v22.patch"
    ]
